...
export const settings = definePluginSettings({
  ...
  autoMoveBack: {
    type: OptionType.BOOLEAN,
    description:
      "Automatically move back to the VC of the followed user when you got moved",
    restartNeeded: false,
    default: false,
  },
  followUserId: {
...
function triggerFollow(
  userChannelId: string | null = getChannelId(settings.store.followUserId),
) {
  if (settings.store.followUserId) {
    ...
    if (userChannelId) {
      ...
      if (userChannelId !== myChanId) {
        ...
        const voiceStates =
          VoiceStateStore.getVoiceStatesForChannel(userChannelId);
        const memberCount = voiceStates
          ? Object.keys(voiceStates).length
          : null;
        ...
        if (channelId !== oldChannelId) {
          ...
          if (
            ...
            const channelVoiceStates =
              VoiceStateStore.getVoiceStatesForChannel(oldChannelId);
            const memberCount = channelVoiceStates
              ? Object.keys(channelVoiceStates).length
              : null;
            if (
              channel.userLimit !== 0 &&
              memberCount !== null &&
              ...
              const users = Object.values(channelVoiceStates).map(
                (x) => x.userId,
              );
